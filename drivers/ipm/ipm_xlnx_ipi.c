...
static int xlnx_ipi_set_enabled(const struct device *ipmdev, int enable)
{
	...
	if (test_bit(config->host_ipi_reg, IPI_IMR, config->remote_ipi_ch_bit) == 0) {
		data->enabled = enable;
		return 0;
	}
...
static int xlnx_ipi_init(const struct device *dev)
{
	...
	volatile uint32_t *reg = (volatile uint32_t *)(conf->host_ipi_reg + IPI_IDR);

	/* disable all the interrupts */
	*reg = 0xFFFFFFFF;

	/* clear status of any previous interrupts */
	reg = (volatile uint32_t *)(conf->host_ipi_reg + IPI_ISR);
	*reg = 0xFFFFFFFF;
